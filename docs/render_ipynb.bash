...
for file in $(cat ipynb_pages.txt); do
    ...
    echo -e "Generated from [notebooks/${file}.ipynb](https://github.com/kipoi/kipoi-interpret/blob/master/notebooks/${file}.ipynb)\n$(cat ${file_out})" > ${file_out}
	...
	sed_replace '![png]('${file}'_files' '![png](/interpret-docs/img/ipynb/'${file}'_files' $file_out
	sed_replace '![svg]('${file}'_files' '![svg](/interpret-docs/img/ipynb/'${file}'_files' $file_out
    fi
done

if [ -d "theme_dir/interpret-docs" ]; then
    rm -r theme_dir/interpret-docs
fi
mkdir -p theme_dir/interpret-docs
ln -sr theme_dir/img theme_dir/interpret-docs/img
