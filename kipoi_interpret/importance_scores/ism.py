...
class Mutation(ImportanceScore):
    ...
    def __init__(self, model, model_input, scores=['diff'],
        ...
        Args:
          ...
          batch_size: batch size for calls to prediction. This is independent
            from the size of batch used with the `score` method.
          score_kwargs: Initialisation keyword arguments for `scores`.
            If not None then it is a list of kwargs dictionaries of the same
            length as `scores`.
          output_sel_fn: Function used to select a model output. Only the
            selected output will be reported as a return value.
          id_value: Which value to use for the identity
          category_axis: Dimension in which the the one-hot category is stored.
            e.g. for a one-hot encoded DNA-sequence array with input
            shape (1000, 4) for a single sample, `category_axis` is 1, for
            (4, 1000) `category_axis` is 0. In the given dimension only one
            value is allowed to be non-zero, which is the selected one.
        """
        self.model = model
    ...
    def score(self, input_batch):
        ...
        scores = []
        for sample_i in range(len(get_model_input(input_batch,
                                                  self.model_input))):
